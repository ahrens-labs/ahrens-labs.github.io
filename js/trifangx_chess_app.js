...
const trifangxChessCloudBridge = { chessData: null, dataLoaded: false };
    ...
    const shopItems = {
      boards: [
        ...
        },
        {
          ...
          preview: '🌅',
          coolness: 10,
          description: 'Solstice Gold — dawn first-light squares over indigo horizon.',
          purchasable: false,
        },
        {
          id: 'season_high_sun',
          ...
          preview: '☀️',
          coolness: 10,
          description: 'Solstice Gold — blazing midday sun over baked terracotta.',
          purchasable: false,
        },
        {
    ...
    const boardPreviewColors = {
      ...
      season_golden_hour: {
        w: 'linear-gradient(155deg,#fffbeb 0%,#ffedd5 38%,#fdba74 100%)',
        b: 'linear-gradient(155deg,#4338ca 0%,#312e81 48%,#1e1b4b 100%)',
      },
      season_high_sun: {
        w: 'linear-gradient(160deg,#fffff5 0%,#fef08a 42%,#fbbf24 78%,#f59e0b 100%)',
        b: 'linear-gradient(160deg,#ea580c 0%,#c2410c 45%,#9a3412 100%)',
      },
    ...
    function isChessAchievementsPage() {
      try {
        return (
          window.TRIFANGX_ACHIEVEMENTS_PAGE === true ||
          /achievements\.html$/i.test(window.location.pathname || '')
        );
      } catch (e) {
        return false;
      }
    ...
    function openTrifangxPageInNewTab(filename) {
    ...
    function showShop() {
      ...
      if (isTrifangxLiveDedicatedPage()) {
        openTrifangxPageInNewTab('chess-shop.html');
        return;
      }
    ...
    function requireChessPregameForNonStatsModal() {
      if (isChessShopPage() || isChessAchievementsPage()) return true;
      try {
    ...
    function trifangxScheduleDeferredTask(fn) {
      const run = function () {
        try {
          fn();
        } catch (e) {
          console.warn('trifangxScheduleDeferredTask', e);
        }
      };
    ...
    function trifangxAfterBoardPaint(fn) {
    ...
    function applyQuickBoardCosmeticsFromCloudOrStorage() {
      const settings =
        dataLoaded && cloudChessData && cloudChessData.settings ? cloudChessData.settings : {};
      let boardStyle = settings.boardStyle || localStorage.getItem('chessboardStyle') || 'classic';
      let pieceStyle = settings.pieceStyle || localStorage.getItem('chessPieceStyle') || 'classic';
      if (dataLoaded && typeof isUnlocked === 'function') {
        if (!isUnlocked('boards', boardStyle)) boardStyle = 'classic';
        if (!isUnlocked('pieces', pieceStyle)) pieceStyle = 'classic';
      }
      ...
      const legalDot =
        settings.legalMoveDotStyle || localStorage.getItem('legalMoveDotStyle') || 'blue-circle';
      applyLegalMoveDotStyle(legalDot);
      currentPageTheme = settings.pageTheme || localStorage.getItem('pageTheme') || 'light';
      currentMoveEffect = settings.moveEffect || localStorage.getItem('moveEffect') || 'default';
      if (isLoggedIn && currentSessionId) {
    ...
    function applyDeferredLobbyStyleAndSettings() {
      ...
      if (tcSelect && !tcSelect.dataset.trifangxTcBound) {
        tcSelect.dataset.trifangxTcBound = '1';
      ...
      if (resetBtn && !resetBtn.dataset.trifangxBound && typeof resetAllAchievements === 'function') {
        resetBtn.dataset.trifangxBound = '1';
    ...
    function loadDeferredSidebarStatsAndAchievements() {
      const savedSound = localStorage.getItem('soundEnabled');
      const soundEl = document.getElementById('sound-effects');
      if (savedSound !== null && soundEl) {
        soundEl.checked = savedSound === 'true';
        soundEnabled = savedSound === 'true';
      }
      if (soundEl && !soundEl.dataset.trifangxBound) {
        soundEl.dataset.trifangxBound = '1';
 ...
 $(document).ready(async function() {
        ...
        if (isChessAchievementsPage()) {
          await bootstrapChessAchievementsPage();
          return;
        }
        ...
        try {
          ...
          if (progress.current >= progress.target && progress.target > 0 && meetsRequirements && meetsNoLosses) {
            ...
            if (typeof cloudChessData !== 'undefined' && cloudChessData) {
              ...
              const now = Date.now();
              const achKey = String(ach.id);
              cloudChessData.achievements[achKey] = {
                at: now,
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      ...
      if (typeof cloudChessData !== 'undefined' && cloudChessData) {
        ...
        Object.keys(parsed).forEach(function (k) {
          norm[String(k)] = parsed[k];
        });
        cloudChessData.achievements = norm;
      }
    } else {
      achievements = [];
      if (typeof cloudChessData !== 'undefined' && cloudChessData) {
    ...
    function saveAchievements() {
      if (typeof cloudChessData !== 'undefined' && cloudChessData) {
        ...
        achievements.forEach(function (id) {
          if (id != null && id !== '') allow.add(String(id));
        });
        Object.keys(o).forEach(function (k) {
          if (!allow.has(String(k))) delete o[k];
        });
        achievements.forEach(function (id) {
          if (id == null || id === '') return;
          const key = String(id);
    ...
    function getAllAchievementsList() {
      return [
        ...
        { id: 'daily_warrior', name: '⚔️ Daily Warrior', desc: 'Make 12 captures in one day (any of your pieces)', category: 'Daily', points: 150, isDaily: true, progress: () => {
        ...
        { id: 'daily_capturer', name: '🎯 Daily Capturer', desc: 'Make 24 captures in one day (any of your pieces)', category: 'Daily', points: 180, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 24 };
          resetDailyStatsIfNeeded();
          const cur = (lifetimeStats.dailyStats.capturesToday || 0) + ((gameStats && gameStats.dailyStats && gameStats.dailyStats.capturesToday) || 0);
          return { current: Math.min(24, cur), target: 24 };
        }},
        ...
        { id: 'daily_center_control', name: '🎯 Center Control', desc: 'Make 22 captures today, all games (any of your pieces)', category: 'Daily', points: 360, isDaily: true, progress: () => {
        ...
        { id: 'daily_survivor', name: '🛡️ Daily Survivor', desc: 'Make 32 captures today, all games (any of your pieces)', category: 'Daily', points: 200, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 32 };
          resetDailyStatsIfNeeded();
          const cur = (lifetimeStats.dailyStats.capturesToday || 0) + ((gameStats && gameStats.dailyStats && gameStats.dailyStats.capturesToday) || 0);
          return { current: Math.min(32, cur), target: 32 };
        }},
        ...
        { id: 'daily_piece_hunter', name: '🎯 Piece Hunter', desc: 'Make 24 captures today, all games (any of your pieces)', category: 'Daily', points: 340, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 24 };
          resetDailyStatsIfNeeded();
          const cur = (lifetimeStats.dailyStats.capturesToday || 0) + ((gameStats && gameStats.dailyStats && gameStats.dailyStats.capturesToday) || 0);
          return { current: Math.min(24, cur), target: 24 };
        }},
        { id: 'daily_elite_capturer', name: '⚔️ Elite Capturer', desc: 'Make 28 captures today, all games (any of your pieces)', category: 'Daily', points: 300, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 28 };
          resetDailyStatsIfNeeded();
          const cur = (lifetimeStats.dailyStats.capturesToday || 0) + ((gameStats && gameStats.dailyStats && gameStats.dailyStats.capturesToday) || 0);
          return { current: Math.min(28, cur), target: 28 };
        }},
        { id: 'daily_pawn_sweeper', name: '🧹 Pawn Sweeper', desc: 'Take 10 enemy pawns today (victim type)', category: 'Daily', points: 300, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 10 };
          resetDailyStatsIfNeeded();
          const c = getDailyPlayerCapturesByTypeMerged();
          return { current: Math.min(10, c.p), target: 10 };
        }},
        { id: 'daily_knight_roundup', name: '🐴 Knight Roundup', desc: 'Take 4 enemy knights today (victim type)', category: 'Daily', points: 340, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 4 };
          resetDailyStatsIfNeeded();
          const c = getDailyPlayerCapturesByTypeMerged();
          return { current: Math.min(4, c.n), target: 4 };
        }},
        { id: 'daily_bishop_ambush', name: '♗ Bishop Ambush', desc: 'Take 4 enemy bishops today (victim type)', category: 'Daily', points: 310, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 4 };
          resetDailyStatsIfNeeded();
          const c = getDailyPlayerCapturesByTypeMerged();
          return { current: Math.min(4, c.b), target: 4 };
        }},
        { id: 'daily_rook_raider_adv', name: '🏰 Rook Raider', desc: 'Take 4 enemy rooks today (victim type)', category: 'Daily', points: 360, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 4 };
          resetDailyStatsIfNeeded();
          const c = getDailyPlayerCapturesByTypeMerged();
          return { current: Math.min(4, c.r), target: 4 };
        }},
        { id: 'daily_queen_snatcher', name: '👑 Queen Snatcher', desc: 'Take 2 enemy queens today (victim type)', category: 'Daily', points: 400, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 2 };
          resetDailyStatsIfNeeded();
          const c = getDailyPlayerCapturesByTypeMerged();
          return { current: Math.min(2, c.q), target: 2 };
        }},
        { id: 'daily_full_deck_hunter', name: '🃏 Mini Deck', desc: 'Capture at least 7 pawns, 2 knights, and 2 rooks today (victim type, all games)', category: 'Daily', points: 420, isDaily: true, progress: () => {
          if (!lifetimeStats || !lifetimeStats.dailyStats) return { current: 0, target: 1 };
          resetDailyStatsIfNeeded();
          const c = getDailyPlayerCapturesByTypeMerged();
          const ok = c.p >= 7 && c.n >= 2 && c.r >= 2;
    ...
    async function bootstrapChessAchievementsPage() {
      ...
      if (pointsEl && typeof computeUnlockedAchievementPointsTotal === 'function') {
        pointsEl.textContent = computeUnlockedAchievementPointsTotal();
      }
      if (loading) loading.style.display = 'none';
      if (body) body.style.display = '';
    ...
    function closeAllAchievements() {
      if (isChessAchievementsPage()) return;
    ...
    function getAllDailyChallengeIds() {
      ...
      const fallbackIds = [
        ...
        'daily_warrior',               // Make 12 captures
        'daily_lightning',              // 2 quick wins (≤25 moves) today
        'daily_capturer',              // Make 24 captures
        ...
        'daily_survivor',              // 32 captures today
        ...
        'daily_center_control',        // 22 captures today
    ...
    async function finalizeLiveChessboardMountAsync(resumeMoveTimerMs) {
      ...
      const soundFxEl = document.getElementById('sound-effects');
      if (soundFxEl) soundEnabled = soundFxEl.checked;
